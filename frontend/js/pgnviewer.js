...
/*var boardSounds=new CTSound({soundPath:"http://chesstempo.com/sounds"});
...
this.boardImagePath="/templates/playzone/images/pieces";
...
startDragPiece = !startDragPiece;
//console.log(this.icon.indexOf('blackpawn') != -1 && this.row == 0);
if(!startDragPiece && ((this.icon.indexOf('whitepawn') != -1 && this.row == 7) || (this.icon.indexOf('blackpawn') != -1 && this.row == 0)))
{
    this.promotePiece = window.prompt('Введите фигуру, в которую Вы хотите превратить пешку (Q - ферзь, R - ладья, B - слон, N - конь)','Q');
    this.insideBoard=false;
    this.endDrag(e);
}
};
ChessPiece.prototype.endDrag=function(e){
startDragPiece = false;    
// к
//console.log(this);
icon = this.icon;
fromSquare      = Array(this.rowFromSquare,this.columnFromSquare);
//console.log(e.pageX + ' ' + e.pageY);
toSquareCoords  = Array(YAHOO.util.Event.getPageX(e)-23,YAHOO.util.Event.getPageY(e)-23);
//console.log(e.clientX + ' ' + e.clientY);
minLeft = 100000;
minTop  = 100000;
//alert(minLeft + ' ' + minTop);
foundId = '';
jQuery('.ct-black-square,.ct-white-square').each(function(){
    leftDif = Math.abs($(this).offset().left - toSquareCoords[0])
    topDif  = Math.abs($(this).offset().top - toSquareCoords[1]);
    if((minLeft >= leftDif) && (minTop >= topDif))
    {
        minLeft = leftDif;
        minTop  = topDif;
...
//console.log(foundId);
capture = '-';
/*$('.board-piece-start-style').each(function(){
    if(
        $(this).css('background-image').indexOf('pawn') == -1 &&  
        $(this).css('visibility') != 'hidden' && 
        $(this).offset().top == $('#'+foundId).offset().top && 
        $(this).offset().left == $('#'+foundId).offset().left &&
        $(this).find('img').attr('src').indexOf('pawn') == -1
      )
    {
        capture = 'x';
    }
})*/
toSquareAr = /playzone\-s(\d+)/.exec(foundId);
toSquare = toSquareAr[1];
toSquare = toSquare.split('');
if(icon.indexOf('pawn') != -1)
{
    firstLetter = 'P';
}
if(icon.indexOf('knight') != -1)
{
    firstLetter = 'N';
}
if(icon.indexOf('bishop') != -1)
{
    firstLetter = 'B';
}
if(icon.indexOf('rook') != -1)
{
    firstLetter = 'R';
}
if(icon.indexOf('queen') != -1)
{
    firstLetter = 'Q';
}
if(icon.indexOf('blackking') != -1 || icon.indexOf('whiteking') != -1)
{
    firstLetter = 'K';
}
letterArrayWhite = 'abcdefgh';
numberArrayWhite = '12345678';
if(!black)
{ 
    letterArray = letterArrayWhite;
    numberArray = numberArrayWhite;
}
else
{
    letterArray = 'hgfedcba';
    numberArray = '87654321';
}
rowFrom     = numberArrayWhite[fromSquare[0]];
columnFrom  = letterArrayWhite[fromSquare[1]];
rowTo       = numberArray[toSquare[1]];
columnTo    = letterArray[toSquare[0]];
prepareMove = firstLetter + columnFrom + rowFrom + capture + columnTo + rowTo;
makeMoveOnBoard(columnFrom + rowFrom + columnTo + rowTo);
if(this.promotePiece)
{
    if(this.promotePiece != 'Q' && this.promotePiece != 'R' && this.promotePiece != 'B' && this.promotePiece != 'N')
    {
        alert('Введите фигуру правильно. Q - ферзь, R - ладья, B - слон, N - конь');
        if(!firstMove)
        {
            pgnViewerGame.board.gotoEnd();
        }
        else
        {
            pgnViewerGame.board.gotoStart();
        }
        return
    }
    else
    {
        prepareMove += this.promotePiece;
    }
}
            
$.get('/play/'+idGame+'/check/'+prepareMove+'/',function(data){
    ...
    // если находимся в режиме ввода варианта
    else if(modeVariation)
    {
        $.get('/play/'+idGame+'/variation/'+prepareMove+'/')
    }
    else
    {
        arMove = /Your last move: ([^ \.]+)/.exec(data);
        jConfirm(
        	'Сделать ход ' + arMove[1].replace('P','') + '?', 
        	'Подтверждение хода', 
        	function(r){
        		if(r)
				{
		            ...
		            $.get('/play/'+idGame+'/move/'+prepareMove+'/',function(data){
...
ChessPiece.prototype.startDrag=function(x,y){
    //$('#pieceDragDiv').show();
this.rowFromSquare    = this.row;
this.columnFromSquare = this.column;
this.insideBoard=true;
var _72=null;
if(this.board.currentMove){
